...
Meridian emits vendor-neutral OpenTelemetry metrics and traces. Run Grafana
...
signal-specific endpoint variables require the full `/v1/traces` and
`/v1/metrics` paths. Never add `OTEL_EXPORTER_OTLP_HEADERS`,
...
4. Confirm the `meridian-api` service in Grafana Cloud Application
   Observability, Tempo traces, and Mimir metrics.
5. Verify that document content, prompts, queries, vectors, and credentials do
   not appear as metric labels or span attributes.
