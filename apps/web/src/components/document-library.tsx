...
import type { CollectionListResponse, DocumentListResponse, DocumentResponse, DocumentUploadAccepted, ReingestionReason } from "@meridian/shared";
...
export function DocumentLibrary({ canReingest }: { canReingest: boolean }) {
  ...
  useEffect(() => { if (!active) setPollCount(0); }, [active]);

  const upload = useCallback(async (file: File) => {
    const validationError = uploadValidationMessage(file);
    if (validationError) { setActionError(new Error(validationError)); return; }
    ...
    try {
      ...
      if (!response.ok) throw await (await import("@/lib/api/contracts")).apiErrorFromResponse(response);
      const accepted = await response.json() as DocumentUploadAccepted;
      setNotice(accepted.deduplicated ? accepted.message : "Upload accepted. Processing continues in the background.");
      setPollCount(0); await documents.mutate();
  ...
  async function reingest(document: DocumentResponse, reason: ReingestionReason) {
    try {
      await meridianRequest("/api/meridian/ingest", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ document_id: document.id, reason }) });
      setNotice("Re-ingestion queued. The current ready generation remains available until the new generation succeeds."); setPollCount(0); await documents.mutate();
    } catch (error) { setActionError(error); }
  }

  return <section className="mx-auto min-h-screen max-w-6xl px-4 py-8 sm:px-8">
    <p className="text-sm font-medium text-primary">Library</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Documents</h1>
    <p className="mt-3 max-w-3xl text-muted-foreground">Manage uploaded knowledge. A deletion hides the document immediately; background cleanup removes its file and vectors safely.</p>
    <div className="mt-6 flex flex-wrap items-end gap-3"><label className="grid gap-1 text-sm font-medium">Collection
      <select className="rounded-md border border-border bg-card p-2" value={collectionId} onChange={(event) => { setCollectionId(event.target.value); setPage(1); setPollCount(0); }}><option value="">All documents</option>{collections.data?.collections.map((collection) => <option key={collection.id} value={collection.id}>{collection.name}</option>)}</select>
...
function DocumentTable({ documents, canReingest, onDelete, onReingest }: { documents: DocumentResponse[]; canReingest: boolean; onDelete: (document: DocumentResponse) => void; onReingest: (document: DocumentResponse, reason: ReingestionReason) => void }) {
  return <div className="overflow-x-auto rounded-lg border border-border"><table className="w-full text-left text-sm"><thead className="bg-muted text-muted-foreground"><tr><th className="p-3">Document</th><th className="p-3">Status</th><th className="p-3">Chunks</th><th className="p-3">Actions</th></tr></thead><tbody>{documents.map((document) => <tr className="border-t border-border" key={document.id}><td className="p-3"><p className="font-medium">{document.filename}</p><p className="text-xs text-muted-foreground">{document.latest_job?.generation ? `Generation ${document.latest_job.generation} · ` : ""}{Math.ceil(document.file_size / 1024)} KiB</p></td><td className="p-3"><span className="rounded-full bg-muted px-2 py-1 text-xs font-medium">{statusLabel(document)}</span>{document.latest_job?.error ? <p className="mt-1 max-w-xs text-xs text-red-700">{document.latest_job.error}</p> : null}</td><td className="p-3">{document.chunk_count ?? "—"}</td><td className="p-3"><div className="flex flex-wrap gap-2"><button className="rounded border border-border px-2 py-1" onClick={() => onDelete(document)} type="button">Delete</button>{canReingest ? <select aria-label={`Re-ingest ${document.filename}`} className="rounded border border-border px-2 py-1" defaultValue="" onChange={(event) => { const value = event.target.value as ReingestionReason; if (value) { onReingest(document, value); event.currentTarget.value = ""; } }}><option value="">Re-ingest…</option>{reasons.map((reason) => <option key={reason.value} value={reason.value}>{reason.label}</option>)}</select> : null}</div></td></tr>)}</tbody></table></div>;
}
