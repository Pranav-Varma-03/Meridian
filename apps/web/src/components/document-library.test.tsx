...
const document = {
  ...
  chunk_count: 4,
  file_size: 1024,
  latest_job: { id: "d43e2f5c-5dc5-49c4-8cbd-9b2a285ac906", status: "ready" as const, attempts: 1, error: null, started_at: null, completed_at: null, generation: 1 },
};

function renderWithFreshCache(children: ReactNode) {
  return render(<SWRConfig value={{ provider: () => new Map(), shouldRetryOnError: false }}>{children}</SWRConfig>);
}
...
describe("document-library workspace", () => {
  it("sends the fixed re-ingestion reason through the BFF only when the capability is present", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path.endsWith("/ingest")) return Response.json({ job_id: "job", status: "queued" }, { status: 202 });
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      "/api/meridian/ingest",
      expect.objectContaining({ method: "POST", body: JSON.stringify({ document_id: document.id, reason: "manual_repair" }) }),
    ));
  });

  it("creates collections through the BFF and surfaces the mutation result", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (init?.method === "POST") return Response.json({ id: "collection", name: "Product", description: null, document_count: 0, created_at: "2026-07-29T10:00:00Z" }, { status: 201 });
      return Response.json({ collections: [], total: 0 });
    });
    vi.stubGlobal("fetch", fetchMock);

    renderWithFreshCache(<CollectionManager />);
    fireEvent.change(await screen.findByLabelText("Collection name"), { target: { value: "Product" } });
    fireEvent.click(screen.getByRole("button", { name: "Create" }));

    await screen.findByText("Collection created.");
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/meridian/collections",
      expect.objectContaining({ method: "POST", body: JSON.stringify({ name: "Product", description: null }) }),
    );
  });
  ...
  it("waits for delete confirmation and only sends the lifecycle deletion after confirmation", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      const path = String(input);
      if (init?.method === "DELETE") return Response.json({ message: "Document deleted and cleanup queued" });
      if (path.includes("/documents")) return Response.json({ documents: [document], total: 1 });
      return Response.json({ collections: [], total: 0 });
    });
    vi.stubGlobal("fetch", fetchMock);
